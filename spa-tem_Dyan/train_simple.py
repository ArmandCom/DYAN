...
'''For distributed model parallel across different machines, it requires inter-node communication, 
right now we don’t have good support for it like DistributedDataParallel wrapper.'''
...
# Create the modelpython3 -m pip install --user virtualenvcd

devices = [1,3]
model = OFModel(Drr, Dtheta, T, PRE, x_fra, y_fra)
...
# TODO divide images in chuncks (use train.py)
count = 0
...
for epoch in range(start_epoch, EPOCH+1):
    ...
    for i_batch, sample in enumerate(dataloader):
        ...
        data = sample['frames'].squeeze(0).cuda(devices[0])
        ...
        ##All from same model
        pred = model(data)
        ##  PRED DIM0 IS DUPLICATED (WHY?) - WHEN CHANGING NUMBER OF GPUS IS STILL DUPLICATED


        # expOut = torch.empty(len(devices),2,20480).cuda(devices[0])
        # for i in range(len(devices)):
        #     expOut[i,:,:] = expectedOut[:,FRA]
        loss = loss_mse(pred, expectedOut[:,FRA])
        loss.backward()
        optimizer.step()
